...
class SnowBoardSummaryView extends WatchUi.View {
    ...
    function onUpdate(dc as Dc) as Void {
        ...
        // A koordinátákat Number-ré kell alakítani (.toNumber())
        dc.drawText(w / 2, (h * 0.12).toNumber(), Graphics.FONT_MEDIUM, "NAPI ÖSSZESÍTŐ", Graphics.TEXT_JUSTIFY_CENTER);
        ...
        drawSummaryRow(dc, w / 2, yStart.toNumber(), "MENETEK:", _runs.toString());
        drawSummaryRow(dc, w / 2, (yStart + ySpace).toNumber(), "LIFTEK:", _lifts.toString());
        drawSummaryRow(dc, w / 2, (yStart + (ySpace * 2)).toNumber(), "TÁVOLSÁG:", _distance.format("%.2f") + " km");
        drawSummaryRow(dc, w / 2, (yStart + (ySpace * 3)).toNumber(), "MAX SEB.:", _maxSpeed.format("%.1f") + " km/h");
        drawSummaryRow(dc, w / 2, (yStart + (ySpace * 4)).toNumber(), "SÜLLYEDÉS:", _totalDescend.format("%d") + " m");

        dc.setColor(Graphics.COLOR_LT_GRAY, Graphics.COLOR_TRANSPARENT);
        dc.drawText(w / 2, (h * 0.88).toNumber(), Graphics.FONT_XTINY, "START a kilépéshez", Graphics.TEXT_JUSTIFY_CENTER);
    }
